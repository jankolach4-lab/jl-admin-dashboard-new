...
-- 4) User daily completions (GLOBAL)
...
-- 4a) User daily completions (PROJECT-SCOPED)
...
-- 5a) User daily changes (PROJECT-SCOPED)
...
-- 5b) User hourly changes for a specific date (PROJECT-SCOPED)
create or replace function public.fn_dashboard_user_hourly_changes_project(p_user_id uuid, p_project text, p_date text)
...
begin perform public.fn_require_admin(); return query select * from public.fn_dashboard_user_daily_completions_project(p_user_id, p_project, p_days); end; $$;
...
begin perform public.fn_require_admin(); return query select * from public.fn_dashboard_user_daily_changes_project(p_user_id, p_project, p_days); end; $$;

create or replace function public.fn_dashboard_guarded_user_hourly_changes_project(p_user_id uuid, p_project text, p_date text)
returns setof public.fn_dashboard_user_hourly_changes_project language plpgsql security definer set search_path = public as $$
begin perform public.fn_require_admin(); return query select * from public.fn_dashboard_user_hourly_changes_project(p_user_id, p_project, p_date); end; $$;
...
grant execute on function public.fn_dashboard_guarded_user_hourly_changes_project(uuid,text,text) to authenticated;